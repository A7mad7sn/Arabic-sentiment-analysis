...
class CNN:
    ...
    def train(self, x_train_features, y_train):
        ...
        if os.path.exists('cnn_model.h5'):
            ...
            self.model = Sequential()
            self.model.add(Embedding(input_dim=max(self.tokenizer.index_word.keys()) + 1, output_dim=200,
                                     input_length=x_train_features.shape[1]))
            self.model.add(Conv1D(filters=100, kernel_size=5, activation='relu'))
            ...
            save_model(self.model, 'cnn_model.h5')
